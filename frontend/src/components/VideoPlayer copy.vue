<!-- VideoPlayer.vue -->
<template>
  <div ref="playerContainer" class="video-player relative aspect-video bg-black overflow-hidden" :class="{ 'fullscreen': isFullscreen }">
    <!-- Server Selection -->
    <div v-if="availableServers.length > 1" class="absolute top-4 right-4 z-10">
    ...
    <div class="relative w-full h-full">
      ...
      <!-- Embed Player (iframe) -->
      <iframe
        v-else-if="videoType === 'embed'"
        class="absolute inset-0 w-full h-full"
        :srcdoc="sanitizedEmbedCode"
        frameborder="0"
        allowfullscreen
      ...
      <!-- Loading Overlay -->
      <div v-if="loading && videoType === 'self-hosted'" class="absolute inset-0 flex items-center justify-center bg-black bg-opacity-50 z-10">
        <div class="loading-spinner"></div>
      </div>

      <!-- Error Overlay -->
      <div v-if="error && videoType === 'self-hosted'" class="absolute inset-0 flex flex-col items-center justify-center bg-black bg-opacity-70 z-10 p-4">
        <div class="text-red-500 text-xl mb-2">{{ error }}</div>
        <button @click="reloadVideo" class="btn btn-primary">Retry</button>
      </div>

      <!-- Controls -->
      <div v-if="videoType === 'self-hosted'" class="absolute bottom-0 left-0 right-0 p-2 bg-gradient-to-t from-black to-transparent opacity-0 hover:opacity-100 transition-opacity duration-300 z-10">
        <!-- Progress Bar -->
        <div class="px-4 py-2">
...
const emit = defineEmits(['timeupdate', 'ended', 'server-change'])
...
const changeServer = (server) => {
  ...
  // Convert server name for the event
  const serverValue = server.toLowerCase() === 'self hosted' ? 'local' : server.toLowerCase()
  emit('server-change', serverValue)
}

// Determine video type based on source
const videoType = computed(() => {
  if (!props.source) return 'self-hosted'
  return props.source.includes('<iframe') || props.source.includes('<IFRAME') ? 'embed' : 'self-hosted'
})
...
const sanitizedEmbedCode = computed(() => {
  if (!props.source || videoType.value !== 'embed') return ''
  return DOMPurify.sanitize(props.source, {
    ADD_TAGS: ['iframe'],
    ADD_ATTR: ['allow', 'allowfullscreen', 'frameborder', 'scrolling', 'src', 'width', 'height', 'marginwidth', 'marginheight']
  })
})
